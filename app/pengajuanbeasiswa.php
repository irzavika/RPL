...
$sqlcek3=mysql_query("select * from batch_pengajuan where nomor_bp = '$username' ");

if($_GET['aksi']=='tambah_pengajuan'){
$id_beasiswa = $_GET['id'];
	if (mysql_num_rows($sqlcek1)==0){
		if (mysql_num_rows($sqlcek3)==0){
			...
			echo "<div class='alert alert-success' align='center'>Anda telah menerima beasiswa</div>";
		}
		}
		else{
		echo "<div class='alert alert-error' align='center'>Pengajuan anda berada dalam list tunggu untuk pemeriksaan</div>";
		}
	} 
	else {
	echo "<audio autoplay><source src='./asset/sound/horse.mp3' type='audio/mpeg'></audio>";
	echo "<a href='index.php?app=databeasiswa&aksi=' class='btn btn-mini'><i class='icon-share-alt'></i> Kembali </a>";
	echo "<br><br> <div class='alert alert-error' align='center'>Anda telah memasukan pengajuan, cuma 1 pengajuan yang dapat di ajukan  </div>";
	}
}
	...
	echo" <form method='POST' action='index.php?app=pengajuanbeasiswa&aksi=edit_data' accept-charset='UTF-8'>
		...
		<tr>
			...
			<td><a href='index.php?app=pengajuanbeasiswa&aksi=' class='btn btn-mini'><i class='icon-repeat'></i> Batal</a></td>
		</tr>";
	echo	"</table>
			</form>";
}
